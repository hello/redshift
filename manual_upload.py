...
def main(args):
    ...
    now = datetime.now()
    try:
        copy_date = datetime.strptime(args[2], "%Y-%m-%d")
    except ValueError, err:
        logging.error("Incorrect date format, use YYYY-MM-DD")
        return
    ...
    table_name = "%s_%s" % (table_prefix, date) # device_sensors_par_2015_08_01
    ...
    if get_db == 'yes':
        ...
        if rds_line_count != line_count:
            ...
            slackbot.post(table_prefix, date,
                "Data file has different number of rows as RDS, die now!")
            sys.exit()
        else:
    ...
    if not os.path.isfile(datafile):
        ...
        slackbot.post(table_prefix, date, "Missing datafile, die now!")
        sys.exit()
    ...
    for org_filename in onlyfiles:
        ...
        if 'gz' in org_filename:
            ...
            timestamp = os.path.getmtime("%s/%s" %(folder, filename))
            modified_dt = datetime.fromtimestamp(timestamp)
    ...
    if num_errors > 0:
        logging.error("Checksum errors. Abort!")
        slackbot.post(table_prefix, date,
                "Manual upload checksum errors, die now! :poop:")
        sys.exit()
    ...
    for filename in file_info:
        ...
        except boto.dynamodb2.exceptions.ItemNotFound, ddb_err:
            logging.error("cannot find entry %s in dynamo logs", filename)
            logging.error("%r", ddb_err)
            continue
    ...
    if okay == 'Y':
        ...
        check_cmd = "%s -c \"SELECT count(1) FROM %s WHERE local_utc_ts >= '%s' AND local_utc_ts <= '%s 23:59:59' \" > tmp_copied" % (REDSHIFT_PSQL, redshift_table, date.replace('_', '-'), date.replace('_', '-'))
        os.system(check_cmd)
    ...
    c_item['stats_end_time'] = datetime.strftime(datetime.now(),
        "%Y-%m-%d %H:%M:%S")
    c_item['step_13_done'] = True
    c_res = c_item.partial_save()

    if c_item['step_02_get_rds_count'] == c_item['step_12_num_rows_copied']:
        slackbot.post(table_prefix, date, 
            "Done. %d rows manually migrated. :boom:" % (
            c_item['step_12_num_rows_copied']))
    else:
        slackbot.post(table_prefix, date,
            "Done. %d rows manually migrated (out of %d). :poop:" % (
            c_item['step_12_num_rows_copied'],
            c_item['step_02_get_rds_count']))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print "Usage: python manual_upload.py [table_prefix] [YYYY-MM-DD]"
        sys.exit()
